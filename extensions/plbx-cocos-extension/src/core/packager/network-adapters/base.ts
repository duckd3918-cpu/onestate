...
export function tiktokBridge(): string {
  return buildPlbxBridge(
    ...
    [
      ...
      // window.gameStart — TikTok SDK calls this to signal the game to begin.
      // Must be defined as a global; validator checks its presence. No-op: game
      // is already running at this point (Cocos boots independently of SDK).
      `window.gameStart = window.gameStart || function() {};`,
      // window.openAppStore — TikTok validator may call this directly as an
      // alternative CTA path alongside playableSDK.openAppStore().
      `window.openAppStore = function() { if (window.playableSDK && playableSDK.openAppStore) { try { playableSDK.openAppStore(); } catch(e) {} } else { window.plbx_html.download(); } };`,
      // Game CTA dispatchers calling window.install()/window.open() directly must
