...
describe('Network Adapters', () => {
  ...
  describe('TikTok adapter', () => {
    ...
    it('should define window.gameStart as no-op for validator lifecycle', () => {
      const adapter = getAdapter('tiktok');
      const builder = new HtmlBuilder(sampleHtml);
      adapter.transform(builder, defaultConfig);
      const html = builder.toHtml();
      expect(html).toContain('window.gameStart = window.gameStart || function()');
    });

    it('should define window.openAppStore as CTA alias for validator', () => {
      const adapter = getAdapter('tiktok');
      const builder = new HtmlBuilder(sampleHtml);
      adapter.transform(builder, defaultConfig);
      const html = builder.toHtml();
      expect(html).toContain('window.openAppStore = function()');
      expect(html).toContain('playableSDK.openAppStore()');
    });
  });
