...
- `src/shared/types.ts` — `NetworkConfig`, `OutputFormat`
- `src/shared/networks.ts:107-115` — текущий `moloco` config (для сравнения)
- `src/core/packager/network-adapters/base.ts:36-50` — `buildPlbxBridge()` (шаблон plbx_html)
- `src/core/packager/network-adapters/base.ts:69-80` — `mraidDeferBootGate()` (viewability gate)
- `src/core/packager/network-adapters/facebook.ts` — текущий Moloco adapter (паттерн для копирования структуры)
- `src/core/packager/network-adapters/mintegral.ts` — adapter с custom plbx_html bridge + forbiddenStrings
- `src/core/packager/network-adapters/index.ts` — registry `CUSTOM_ADAPTERS`
- `src/core/packager/packager.ts:14-189` — `packageForNetworks()` main loop
- `src/core/packager/runtime-loader.ts:732-852` — `generateFullHtml()` (single-HTML inliner)
...
`OutputFormat` type расширяется в `src/shared/types.ts`:
```ts
...
В `src/shared/networks.ts` (после `moloco` block):
```ts
molocoV2: {
  id: 'molocoV2',
...
### 5. MolocoV2Adapter (`src/core/packager/network-adapters/moloco-v2.ts`)
...
### 6. Launcher builder (`src/core/packager/launcher-builder.ts` — НОВЫЙ файл)
...
`tests/core/packager/moloco-v2.test.ts` — новый файл. Покрытие:
