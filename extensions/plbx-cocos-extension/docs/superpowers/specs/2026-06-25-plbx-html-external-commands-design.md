...
- Buttons in the standalone validator (`src/core/preview/server.ts`). Scope is
...
## API — build side (`src/core/packager/network-adapters/base.ts`)
...
- `src/core/packager/network-adapters/base.ts` — add `expose` + `external_commands`
  to `buildPlbxBridge` (inherited by all buildPlbxBridge networks).
- `src/core/packager/network-adapters/mintegral.ts`,
  `src/core/packager/network-adapters/moloco-v2.ts` — explicit registry copy
...
- `tests/core/packager/plbx-external-commands.test.ts` — `expose` unit tests +
