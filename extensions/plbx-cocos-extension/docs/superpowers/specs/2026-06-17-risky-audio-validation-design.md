...
### 1. `src/core/packager/audio-format-check.ts` (NEW, pure / unit-testable)
...
### 2. `src/core/packager/packager.ts` (MODIFY)
...
`tests/core/packager/audio-format-check.test.ts` (new):
...
- `src/core/packager/audio-format-check.ts` — NEW
- `src/core/packager/packager.ts` — detect + warn + inject marker
- `src/core/preview/server.ts` — parse marker → `risky_audio` check
