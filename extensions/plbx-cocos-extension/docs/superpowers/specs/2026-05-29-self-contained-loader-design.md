...
Split `src/core/packager/runtime-loader.ts` (currently ~1025 lines) into a `loader/` directory, each module emitting its slice of the loader JS string:

- `src/core/packager/loader/unpack.ts` — `plbx_load`, JSZip runtime, `__plbx_res` population.
- `src/core/packager/loader/modules.ts` — `resolve`/`instantiate` overrides, `plbx_eval`, `plbx_reg_search`, `_PLBX_URL`.
- `src/core/packager/loader/assets.ts` — downloader handlers, `plbx_getRes`, `_PlbxLocalRequest`, mime/data-uri helpers.
- `src/core/packager/loader/lifecycle.ts` — `plbx_boot`, `plbx_boot_engine`, mraid defer-boot gate.
- `src/core/packager/runtime-loader.ts` — orchestrator: `generateRuntimeLoader(options)` composes the four slices (plus legacy path during transition).
