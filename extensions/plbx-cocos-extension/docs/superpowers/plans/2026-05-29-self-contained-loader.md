...
The current `src/core/packager/runtime-loader.ts` already implements several super-html ideas:
...
**Create:**
- `src/core/packager/loader/legacy.ts` — the current loader code moved verbatim (the `'systemjs'` branch). Exports `generateLegacyLoader(options)`.
- `src/core/packager/loader/shared.ts` — emitted JS helpers shared by both loaders: `_suffixMatch`, `_isVirtualScheme`, `_base64ToArrayBuffer`, `_stringToArrayBuffer`, MIME map, `_getMime`, `_toDataUri`. (Pure string emitters.)
- `src/core/packager/loader/unpack.ts` — `plbx_load`: ZIP → `__plbx_res`/`__plbx_bin` + lifecycle signaling. Exports `generateUnpack(options)`.
- `src/core/packager/loader/modules.ts` — `plbx_reg_search`, `_PLBX_URL`, `resolve`/`instantiate` overrides. Exports `generateModules(options)`.
- `src/core/packager/loader/assets.ts` — `plbx_getRes`, `_PlbxLocalRequest`, `cc.assetManager.downloader` handlers (image/audio/video/font). Exports `generateAssets(options)`.
- `src/core/packager/loader/lifecycle.ts` — `plbx_boot`, `plbx_boot_engine`, mraid defer-boot gate hookup. Exports `generateLifecycle(options)`.

**Modify:**
- `src/core/settings.ts` — add `loaderMode` + `legacyLoaderNetworks` to `ProjectSettings` + `DEFAULT_SETTINGS`.
- `src/shared/types.ts` — add `loaderMode?` to `PackageConfig`.
- `src/core/packager/runtime-loader.ts` — `RuntimeLoaderOptions` gains `mode`; `generateRuntimeLoader` dispatches by mode; `generateFullHtml` accepts + threads `loaderMode`.
- `src/core/packager/packager.ts:185` — pass effective per-network mode into `generateFullHtml`.
- `tests/core/packager/runtime-loader.test.ts` — make the 37 content assertions mode-aware.
...
- Modify: `src/shared/types.ts:30-36` (`PackageConfig`)
...
In `src/shared/types.ts`, add to `PackageConfig`:
...
```bash
git add src/core/settings.ts src/shared/types.ts tests/core/settings.test.ts
...
Run: `npx vitest run tests/core/packager/runtime-loader.test.ts -t 'default mode emits'`
...
Create `src/core/packager/loader/legacy.ts`. Move `generateUnpackCode`, `generatePatchCode`, and all their emitted-JS bodies (current `runtime-loader.ts` lines 47-822) into it verbatim. Export a single composed function:
...
In `src/core/packager/runtime-loader.ts`, extend the options and dispatch:
...
Create `src/core/packager/loader/index.ts`:
...
Run: `npx vitest run tests/core/packager/runtime-loader.test.ts`
...
```bash
git add src/core/packager/loader/legacy.ts src/core/packager/loader/index.ts src/core/packager/runtime-loader.ts tests/core/packager/runtime-loader.test.ts
...
**Files:**
- Create: `src/core/packager/loader/shared.ts`
- Test: `tests/core/packager/loader-shared.test.ts`
...
Run: `npx vitest run tests/core/packager/loader-shared.test.ts`
...
```bash
git add src/core/packager/loader/shared.ts tests/core/packager/loader-shared.test.ts
...
**Files:**
- Create: `src/core/packager/loader/modules.ts`
- Test: `tests/core/packager/loader-modules.test.ts`
...
Run: `npx vitest run tests/core/packager/loader-modules.test.ts`
Expected: FAIL — module not found.
...
```bash
git add src/core/packager/loader/modules.ts tests/core/packager/loader-modules.test.ts
...
**Files:**
- Create: `src/core/packager/loader/assets.ts`
- Test: `tests/core/packager/loader-assets.test.ts`
...
Run: `npx vitest run tests/core/packager/loader-assets.test.ts`
Expected: FAIL — module not found.
...
```bash
git add src/core/packager/loader/assets.ts tests/core/packager/loader-assets.test.ts
...
**Files:**
- Create: `src/core/packager/loader/unpack.ts`
- Create: `src/core/packager/loader/lifecycle.ts`
- Test: `tests/core/packager/loader-unpack.test.ts`
...
Run: `npx vitest run tests/core/packager/loader-unpack.test.ts`
...
```bash
git add src/core/packager/loader/unpack.ts src/core/packager/loader/lifecycle.ts tests/core/packager/loader-unpack.test.ts
...
**Files:**
- Modify: `src/core/packager/loader/index.ts`
- Test: `tests/core/packager/runtime-loader.test.ts`

- [ ] **Step 1: Write the failing test**

Append to `tests/core/packager/runtime-loader.test.ts`:
...
Run: `npx vitest run tests/core/packager/runtime-loader.test.ts -t 'no global XHR patch'`
...
Run: `npx vitest run tests/core/packager/runtime-loader.test.ts tests/core/packager/html-builder.test.ts`
Expected: PASS.

- [ ] **Step 7: Commit**

```bash
git add src/core/packager/loader/index.ts src/core/packager/runtime-loader.ts tests/core/packager/runtime-loader.test.ts
...
**Files:**
- Modify: `src/core/packager/packager.ts:185`
...
`legacyLoaderNetworks` is not on `PackageConfig` yet — add `legacyLoaderNetworks?: string[]` to `PackageConfig` in `src/shared/types.ts` (alongside `loaderMode?` from Task 1). Then pass into the `generateFullHtml({...})` call at :185:
...
```bash
git add src/core/packager/packager.ts src/shared/types.ts src/main.ts
